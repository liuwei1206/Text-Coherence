...
class PretrainedForTextClassification(PreTrainedModel):
    ...
    def __init__(self, config, params):
        ...
        self.doc_pooling_type = params['doc_pooling_type']
        self.para_pooling_type = params['para_pooling_type']
        self.sent_pooling_type = params['sent_pooling_type']

        if self.model_type.upper() == 'BERT':
            self.text_encoder = BertModel(config)
        elif self.model_type.upper() == 'ROBERTA':
            self.text_encoder = RobertaModel(config)
        elif self.model_type.upper() == 'ALBERT':
            self.text_encoder = AlbertModel(config)
        elif self.model_type.upper() == 'XLNET':
            self.text_encoder = XLNetModel(config)

        # for document-paragraph hierarchy and document-sentence hierarchy
        if self.doc_type.upper() == "DOCUMENT-PARAGRAPH":
            self.para_transformer = Transformer(
                d_model=params['para_d_model'],
                nhead=params['para_nhead'],
                num_layers=params['para_num_layers']
            )
        elif self.doc_type.upper() == "DOCUMENT-SENTENCE":
        ...
        if self.doc_pooling_type.upper() == "ATTENTION":
            self.doc_attn_W1 = nn.Linear(config.hidden_size, config.hidden_size)
            self.doc_attn_w2 = nn.Parameter(torch.zeros(config.hidden_size))
    ...
    def forward(
        ...
        if mode.upper() == "DOCUMENT":
            ...
            doc_outputs = torch.index_select(sequence_outputs, dim=1, index=torch.tensor(0)).squeeze()
            ...
            print("paragraph pooling type: ", self.para_pooling_type)
            ...
            para_sequence_outputs = self.para_transformer(
                src=para_outputs, mask=doc_para_attention_mask.float()
            )

            print("document pooling type: ", self.doc_pooling_type)
            if self.doc_pooling_type.upper() == "CLS":
                ...
                doc_outputs = masked_para_sequence_outputs / para_length.unsqueeze(-1) # [N, D]
            elif self.doc_pooling_type.upper() == "ATTENTION":
                alpha = self.doc_attn_W1(para_sequence_outputs) # [N, PN, D]
                tan = nn.Tanh()
                alpha = tan(alpha)
                alpha = torch.matmul(alpha, self.doc_attn_w2).squeeze() # [N, PN]
            ...
            print("sentence pooling type: ", self.sent_pooling_type)
            ...
            sent_sequence_outputs = self.sent_transformer(
                src=sent_outputs, mask=doc_sent_attention_mask.float()
            )
            ...
            if self.doc_pooling_type.upper() == "CLS":
                ...
                alpha = self.doc_attn_W1(sent_sequence_outputs)  # [N, SN, D]
                alpha = nn.Tanh()(alpha)
                alpha = torch.matmul(alpha, self.doc_attn_w2).squueze()  # [N, S]
